...
async def person(message):
    reds = ["👫Обираємо стать", "🗄Готуємо професію", "💊Лікуємо здоров'я", "🧳Набираємо багаж", "📝Навчаємо освіти", "🃏Роздаємо картки дії"]
    ...
    await bot.send_message(message.chat.id, str("🃏Карта дії №1: ") + str(random.choice(abilities.actioncard)))
    await bot.send_message(message.chat.id, str("🃏Карта дії №2: ") + str(random.choice(abilities.actioncard)))
